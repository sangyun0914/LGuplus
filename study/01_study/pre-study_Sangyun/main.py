...
faceCascade = cv2.CascadeClassifier("study/01_study/pre-study_SiHeon/haarcascade_frontalface_default.xml")
...
while cap.isOpened():    #webcam 연결 시 실행
    ...
    video1 = cv2.resize(frame, (0, 0), None, .5, .5)   #얼굴인식 및 블러
    video2 = cv2.resize(frame, (0, 0), None, .5, .5)   #yolo
    video3 = cv2.resize(edges, (0, 0), None, .5, .5)    #edge
    video4 = cv2.resize(frame, (0, 0), None, .5, .5)


    bbox, label, conf = cv.detect_common_objects(video2, model='yolov4-tiny')
