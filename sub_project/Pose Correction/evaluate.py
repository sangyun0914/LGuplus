...
def evaluate_angle(pose_seq, exercise):
    ...
    if exercise == 'squat':
        ...
        return _lunge(pose_seq)
    elif exercise == 'pushup':
        return _pushup(pose_seq)
    else:
...
def _squat(pose_seq):
    ...
    left_joints = [(pose[PoseLandmark.LEFT_SHOULDER], pose[PoseLandmark.LEFT_HIP], pose[PoseLandmark.LEFT_KNEE], pose[PoseLandmark.LEFT_ANKLE]) for pose in pose_seq]
    right_joints = [(pose[PoseLandmark.RIGHT_SHOULDER], pose[PoseLandmark.RIGHT_HIP], pose[PoseLandmark.RIGHT_KNEE], pose[PoseLandmark.RIGHT_ANKLE]) for pose in pose_seq]
    
    # filter out data points where a part does not exist
    left_joints = [left_joint for left_joint in left_joints if all(part.visibility > constants.VIS_THRESHOLD for part in left_joint)]
    right_joints = [right_joint for right_joint in right_joints if all(part.visibility > constants.VIS_THRESHOLD for part in right_joint)]

    left_torso = np.array([(joint[0].x - joint[1].x, joint[0].y - joint[1].y, joint[0].z - joint[1].z) for joint in left_joints])
    left_vertical = np.array([(joint[1].x, joint[1].y, joint[1].z - 1) for joint in left_joints])
